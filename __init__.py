...
def load(app: Flask) -> None:
    ...
    with app.app_context():
        # Drop the tables
        if ContainerInfoModel.__table__.exists(db.engine):
            ContainerInfoModel.__table__.drop(db.engine)
        if ContainerChallengeModel.__table__.exists(db.engine):
            ContainerChallengeModel.__table__.drop(db.engine)
