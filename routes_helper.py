...
def create_container(container_manager, challenge_id, user_id, team_id, docker_assignment):
    ...
    try:
        ...
        log("containers_debug", format="CHALL_ID:{challenge_id}|Creating new Docker container",
                challenge_id=challenge_id)
  ports = json.loads(challenge.ports)
  port_bindings = {}
  for internal_port in ports:
   port_bindings[str(internal_port)] = None

  created_container = container_manager.create_container(
   challenge.image, port_bindings, challenge.command, challenge.volumes)
    except Exception as err:
  ...
  ports_mapping = {}
  for internal_port in ports:
   external_port = container_manager.get_container_port(created_container.id, internal_port)
   ports_mapping[internal_port] = external_port
  ...
  if not ports_mapping:
   ...
   return json.dumps({"status": "error", "error": "Could not get port"})
    ...
    new_container = ContainerInfoModel(
        ...
        user_id=user_id,
        team_id=team_id,
        ports=json.dumps(ports_mapping),
