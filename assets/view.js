...
function container_request(challenge_id) {
 ...
 xhr.onload = function () {
  ...
  if (data.error !== undefined) {
   ...
   let ports_info = "";
   for (const [internal_port, external_port] of Object.entries(data.ports)) {
    if (data.hostname.startsWith("http")) {
     ports_info += `<a href="${data.hostname}:${external_port}" target="_blank">${data.hostname}:${external_port}</a> (Port ${internal_port})<br>`;
    } else {
     ports_info += `${data.hostname} ${external_port} (Port ${internal_port})<br>`;
    }
   } else {
    connectionInfo.innerHTML = ports_info
   }
